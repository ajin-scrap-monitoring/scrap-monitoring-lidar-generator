"""Deterministic full-jitter reconnect backoff for observation streaming."""
...
class ReconnectBackoff:
    """Increase the reconnect delay cap until a successful observation send."""
    ...
    def consecutive_failures(self) -> int:
        """Return failures observed since the last successful send."""
    ...
    def reset_after_success(self) -> None:
        """Reset the cap after a successful observation send."""
        self._current_delay_cap_s = self._initial_delay_s
        self._consecutive_failures = 0


def _derive_seed(seed: int) -> int:
    payload = b"observation\0" + seed.to_bytes(8, byteorder="big") + b"reconnect-jitter"
