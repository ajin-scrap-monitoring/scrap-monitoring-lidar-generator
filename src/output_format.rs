...
#[derive(Serialize)]
pub(crate) struct ObservationSurfaceDocument<'a> {
    cell_size_m: f64,
...
impl<'a> From<&'a SurfaceSnapshot> for ObservationSurfaceDocument<'a> {
...
#[derive(Serialize)]
pub(crate) struct DiagnosticsSurfaceDocument<'a> {
    snapshot_at_s: f64,
    boundary_xy_m: BoundaryCoordinates<'a>,
    floor_z_m: f64,
    top_z_m: f64,
    cell_size_m: f64,
    shape: [usize; 2],
...
impl<'a> DiagnosticsSurfaceDocument<'a> {
    pub(crate) fn new(snapshot: &'a ScenarioModelSnapshot) -> Self {
        let surface = &snapshot.surface;
        let (height, width) = surface.shape();
        Self {
            snapshot_at_s: snapshot.state.elapsed_s,
            boundary_xy_m: BoundaryCoordinates(surface),
...
struct BoundaryCoordinates<'a>(&'a SurfaceSnapshot);

impl Serialize for BoundaryCoordinates<'_> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let vertices = self.0.boundary().vertices();
        let mut coordinates = serializer.serialize_seq(Some(vertices.len()))?;
        for vertex in vertices {
            coordinates.serialize_element(&vertex.coordinates())?;
        }
        coordinates.end()
    }
}
