...
| 담당자 gRPC over UDS scan 출력 | 구현 및 담당자 source 직접 검증 완료 | `contracts/lidar/v1/`, `edge-platform-integration/` |
| 적재 모델 관찰 stream | 생성기 출력 구현 완료 | `docs/observation.md` |
| ARM64 image 배포 | 자동 build 및 image 검증 구성 완료 | `docs/deployment.md` |

현재 source version은 0.8.0이다. 생성기는 하나의 적재 모델에서 정확히 2개 sensor scan을
만들고 담당자 `LidarScanSource.SubscribeScans`와 같은 sensor별 gRPC(Google Remote Procedure
Call) over UDS(Unix Domain Socket) endpoint를 제공한다. `lidar-processing`은 생성기의 환경
JSON을 읽지 않으며 exporter가 같은 공개 합성 환경에서 담당자 처리 설정을 만든다.

scan 계약 전환의 완료 조건은 다음과 같다.

- 담당자 Proto와 고정 source metadata의 일치.
...
- 담당자 상태 파일 이름과 정상 상태 표현의 일치.
- 실제 담당자 설정 loader와 `ProcessingEngine`의 두 sensor frame 수락.
...
| 순서 | 작업 | 선행 조건 | 완료 기준 |
| --- | --- | --- | --- |
| 1 | 실제 edge 배포 구성 결합 | 담당자 전체 edge 설정과 image 준비 | 두 UDS 구독, 두 상태 파일과 처리 `GOOD` 상태 |
| 2 | Raspberry Pi 5 장기 공유 부하 검증 | SDK 변환기와 높이 계산 process 동시 실행 | CPU, 메모리, sequence gap과 처리 지연 상한 확정 |
| 3 | 실제 S2E 비교 검증 | sensor 2대와 비공개 관측 저장소 준비 | 실제 SDK frame과 생성 frame의 의미 및 분포 차이 기록 |

첫 번째 작업은 `edge-platform-integration/`의 exporter와 수락 명령을 사용한다. 두 번째 작업은
생성기만 자원을 독점한다고 가정하지 않고 SDK 변환기, `lidar-processing`과 다른 edge process를
함께 실행한다. 성능 상한은 측정 전에 임의로 확정하지 않는다. 세 번째 작업의 원시 frame,
...
담당자 Proto 또는 처리 설정이 바뀌면 고정 source commit, 로컬 Proto, 생성 binding, exporter,
