...
`measurement.SpatialDistortionTimeline`은 모든 센서가 공유하는 낙하물 사건을 시뮬레이션 시간순으로 생성한다. 후보 사건은 투입 속도 상한의 Poisson process에서 만들고 해당 시각의 실제 투입 속도 비율로 선별한다. 사건 중심은 활성 투입구의 더미 확산 범위 안에서 선택하며, 기준 표면 교차점의 평면 투영이 사건 반경 안에 있고 측정점 시각이 사건의 반개구간 안에 있을 때 거리 감소 후보를 만든다. 완료되지 않은 가장 오래된 회전보다 먼저 끝난 사건은 제거한다.
