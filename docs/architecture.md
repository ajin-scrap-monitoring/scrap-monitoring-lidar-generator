...
높이 계산 프로세스와 공유하는 환경, scan 및 응답 계약은 `height-calculation-contract-proposal/`에서 JSON Schema Draft 2020-12와 합성 fixture로 관리한다. `contracts/v1/`은 생성 실행 설정 및 품질 분포 계약과 공유 계약의 호환 진입 경로를 관리한다. `configuration`은 각 계약의 구조 규칙과 JSON Schema로 표현할 수 없는 다각형, 방향벡터, 구간 순서 및 참조 입력 사이의 규칙을 함께 검증한다.
...
각 적재 및 수거 갱신은 국소 변화와 요철 적용 뒤 안식각 기반 경사 이완을 수행한다. 이완은 직교 및 대각선 이웃 node 사이의 높이 차이가 합성 안식각 35도 이내가 되도록 높은 node의 부피를 낮은 node로 옮긴다. 한 반복의 이웃별 이동량을 동시에 적용하여 방향 편향을 막고, node별 면적 가중치를 사용하여 적분 부피와 바닥 및 상단 경계를 보존한다. 계산은 갱신당 최대 32회로 제한하므로 같은 입력과 seed에서 같은 표면을 만든다.
...
`measurement.SensorRotationScheduler`는 센서마다 독립된 회전 상태와 `scan_id`를 관리한다. 각 센서는 실행 seed와 `sensor_id`에서 안정적으로 파생한 시작 각도를 사용하며, 다른 센서의 scheduler 진행 여부가 해당 센서의 결과를 바꾸지 않는다. SDK는 첫 노드의 sync bit를 보장하지만 해당 노드의 수치 각도를 고정하지 않으므로 소비자는 첫 각도를 회전 경계의 고정값으로 가정하지 않는다.

RPLIDAR SDK 이후 표현과 생성 데이터의 정합성 경계는 [`sdk-compatibility.md`](sdk-compatibility.md)가 정본이다. 생성기는 제조사 통신과 SDK 수신을 구현하지 않으며 운영 image에 SDK를 포함하지 않는다.
...
contracts/
  ...
  observation/
    v1/
height-calculation-contract-proposal/
  v1/
docs/
  architecture.md
  deployment.md
