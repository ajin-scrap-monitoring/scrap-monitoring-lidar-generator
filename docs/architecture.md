...
`measurement.MeasurementGenerator`는 기준 스크랩 표면을 관측한 측정점에 설정 확률로 반사 경로 오류를 먼저 적용한다. 거리 감소량은 설정 범위와 측정 가능한 거리의 교집합에서 선택하며 벽, 바닥과 무효 측정은 이 오류에서 제외한다. 이어서 유효 거리에 설정 한계 안의 절단 정규분포 오차를 적용하고 측정 범위를 다시 검사한다. 범위 밖 결과와 기준 무효 거리는 거리 0으로 만들며, 최종 거리 유효성에 맞는 센서별 누적 품질 분포에서 8비트 값을 선택한다. 최종 각도, 거리와 품질은 읽기 전용 배열로 보관한다.
