...
검증 대상은 source revision `04e456d2983a0c71e073a83c3717adc94d80bce0`의 `v0.4.0`
ARM64 image다. Release asset의 불변 image 참조는 다음과 같다.

```text
ghcr.io/ajin-scrap-monitoring/scrap-monitoring-lidar-generator@sha256:ea685aee69ef92d1e3a42c35aa549fbdc39b809ef2567f2797792a3eeaf25ddd
```

Raspberry Pi 5 8GB에서 공개 합성 2센서 설정, scan ACK test double과 관찰 JSON Lines 수신
test double을 같은 Docker network에서 실행했다. 생성 container에는 CPU 2 core 상한을
적용하고 30초 wall-clock 구간을 검증했다. 설정 경로, scan 및 관찰 endpoint, 관찰 주기와
진단 설정은 8개 환경변수를 담은 임시 환경변수 파일로 주입했다.
...
| scan 폐기 및 연결 실패 | 0 |
| 관찰 송수신 | 32 record, 폐기 및 연결 실패 0 |
| 생성 process CPU | 82.02퍼센트 |
| 수신 test double CPU | 12.14퍼센트 |
| 검증 후 장비 상태 | load average 0.02, MemAvailable 7,832,304 kB, 53.2 C, throttling `0x0` |

두 container는 종료 코드 0이고 OOM(Out Of Memory) 종료가 없었다. 현재 장비 kernel은
Docker memory cgroup 사용량을 제공하지 않아 `docker stats`가 `0B / 0B`를 반환했으며
container 메모리 사용량과 상한은 검증하지 못했다. Release image는 비 root 사용자
`10001:10001`로 실행됐으며 version `0.4.0`, source revision과 ARM64 metadata를 장비에서
확인했다. 검증 container와 network 및 원격 임시 파일은 제거했고 Release image는 장비에
digest로 pull된 상태다. 실제 scan 수신 프로그램, 별도 시각화 장비, 다른 edge process와
장기 동시 실행은 포함하지 않는다.
