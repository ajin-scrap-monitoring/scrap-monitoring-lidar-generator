...
Rust 기본 구현은 `src/`, Python 기준 구현은 `src/scrap_monitoring_lidar_simulator/`, 자동 검증은 `tests/`, 공개 가능한 합성 입력 예시는 `examples/`에 둔다. OCI image는 Rust 단일 실행 파일을 기본 실행 경로로 사용하고 Python 구현은 동등성 및 개발 검증에 사용한다. 새로운 최상위 경계가 필요하면 코드와 함께 아키텍처 문서를 갱신한다.
...
공개 합성 환경의 정본은 `examples/environment.v1.json`과 이를 참조하는 `examples/generator.v2.json`의 조합이다. 전자는 적재 공간 치수, 경계 형상, 바닥 및 상단 높이와 센서 위치 및 방향을 정의하고, 후자는 투입구 위치와 시나리오 설정을 정의한다.
