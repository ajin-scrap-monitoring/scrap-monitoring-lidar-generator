...
선택적 개발 관찰 기능을 추가할 때는 생성기가 제공하는 적재 모델 snapshot을 기존 scan 전송과 별도 경계로 유지한다. 관찰 기능은 기본 비활성화하고 비동기 bounded 정책으로 생성 및 기존 scan 전송을 막지 않게 한다. 렌더링과 MP4 생성은 운영 이미지와 엣지 실행 경로에 포함하지 않는다.
