...
Rust 후보 구현은 `src/`, Python 기본 구현은 `src/scrap_monitoring_lidar_simulator/`, 자동 검증은 `tests/`, 공개 가능한 합성 입력 예시는 `examples/`에 둔다. 단계 5까지 Python 구현과 현재 OCI image entrypoint를 기본 실행 경로로 유지한다. 새로운 최상위 경계가 필요하면 코드와 함께 아키텍처 문서를 갱신한다.
